...
2. **First launch opens `/setup`** — event name, the Wi-Fi crew join, the
   ...
   It is not the event PIN and it is not shown again. Missed it anyway?
   **Menu bar → Open the admin panel** opens the panel without it, once, and
   you can set a new one under **Admin → This box**.
...
3. **Write down the admin password.** The box mints one on first start and
   ...
   the event PIN. Change it in **Admin → This box**. If it is ever lost,
   `sudo cat /var/lib/crewbox/admin-link.json` holds a link that opens the
   panel without it, once (if it says `localhost`, put the box's address
   there to open it from another machine). Or set `ADMIN_PASSWORD` in the
   service file and restart — that overrides the stored one for as long as it
   is there.
...
| Locked out of Admin        | No restart needed: `sudo cat /var/lib/crewbox/admin-link.json` shows a link that opens the panel once without the password (if it says `localhost`, put the box's address there to open it from another machine), then set a new password in **Admin → This box**. On a Mac or Windows box, **Open the admin panel** in its menu does the same. Or set `ADMIN_PASSWORD=…` in `/etc/systemd/system/crewbox.service`, `systemctl daemon-reload && systemctl restart crewbox`. It overrides the stored password. Nobody loses their session; only the panel re-locks. |
